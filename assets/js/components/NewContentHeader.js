...
module.exports = class NewContentHeader {
  ...
  constructor($elm, _window = window, doc = document) {
    ...
    this.doc = doc;
    this.$elm = $elm;
    this.$elm.classList.add('new-content-header--js');

    this.authors = $elm.querySelectorAll('.new-content-header__author_list_item');
    this.institutions = $elm.querySelectorAll('.new-content-header__institution_list_item');

    if (this.authors.length > 0) {
      this.authors[this.authors.length - 1].classList.add('new-content-header__author_list_item--last');
    }
    if (this.institutions.length > 0) {
      this.institutions[this.institutions.length - 1].classList.add('new-content-header__institution_list_item--last');
    }
  ...
  handleAnyExcessItems(itemType, items) {
    let toggle = this.$elm.querySelector('.new-content-header__item_toggle');
  ...
  markLastNonExcessItem(itemType, items) {
    ...
    [].forEach.call(items, function (item, i) {
      ...
      items[i].querySelector('.new-content-header__' + itemType).classList.remove('new-content-header__' + itemType + '--last-non-excess');
    ...
    if (lastShownIndex !== null && lastShownIndex > -1) {
      let lastShown = items[lastShownIndex].querySelector('.new-content-header__' + itemType);
      if (lastShown) {
        lastShown.classList.add('new-content-header__' + itemType + '--last-non-excess');
      }
    }
  ...
  buildSeeMoreLessToggle(itemType) {
    ...
    toggle.classList.add('new-content-header__item_toggle', 'new-content-header__item_toggle--' + itemType);
    toggle.innerHTML = '&nbsp;&hellip;&nbsp;' + toggleOnText;
    toggle.addEventListener('click', e => {
      let target = e.target;
      if (target.innerHTML.indexOf(toggleOnText) > -1) {
        [].forEach.call(this.doc.querySelectorAll('.new-content-header__item_toggle'), item => {
    ...
    this.$elm.querySelector('.new-content-header__' + itemType + '_list').appendChild(toggle);
  }
